...
export class CopayApp {
  ...
  private handleDeepLinks() {
    ...
    if (lastUrl && lastUrl !== '') {
      ...
      this.events.subscribe('AllWalletsUpdated', () => {
        this.events.unsubscribe('AllWalletsUpdated');
  ...
  private onOpenNW(pathData) {
    if (pathData.indexOf('tituscoincash:/') != -1) {
      this.logger.debug('Titus Coin Cash URL found');
      this.handleOpenUrl(pathData.substring(pathData.indexOf('tituscoincash:/')));
    } else if (pathData.indexOf('tituscoin:/') != -1) {
      this.logger.debug('Titus Coin URL found');
      this.handleOpenUrl(pathData.substring(pathData.indexOf('tituscoin:/')));
