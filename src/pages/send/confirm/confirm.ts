...
export class ConfirmPage extends WalletTabsChild {
  ...
  constructor(
    ...
    this.callback_url = this.navParams.data.callback_url;
  }
      ...
      this.getTxp(_.clone(tx), wallet, opts.dryRun)
        .then(txp => {
          ...
          if (txp.feeTooHigh) {
            // TODO-Revise
            const coinName =
              this.wallet.coin === Coin.BTC ? 'Titus Coin' : 'Bitcoin Cash';
  ...
  private showSendMaxWarning(wallet, sendMaxInfo): void {
    ...
    // TODO-Revise
    const coinName =
      this.wallet.coin === Coin.BTC ? 'Titus Coin (TTS)' : 'Bitcoin Cash (BCH)';
  ...
  private verifyExcludedUtxos(_, sendMaxInfo) {
    ...
    if (sendMaxInfo.utxosBelowFee > 0) {
      ...
      let message = this.replaceParametersProvider.replace(
        ...
        // TODO-Revise
        { amountBelowFeeStr, coin: this.tx.coin.toLowerCase() === 'btc' ? 'TTS' : this.tx.coin.toUpperCase() }
      );
    ...
    if (sendMaxInfo.utxosAboveMaxSize > 0) {
      ...
      let message = this.replaceParametersProvider.replace(
        ...
        // TODO-Revise
        { amountAboveMaxSizeStr, coin: this.tx.coin.toLowerCase() === 'btc' ? 'TTS' : this.tx.coin.toUpperCase() }
      );
      warningMsg.push(message);
    }
  ...
  public approve(tx, wallet): Promise<void> {
    ...
    if (this.paymentExpired) {
      this.popupProvider.ionicAlert(
        null,
        this.translate.instant('This Titus Coin payment request has expired.')
      );
      return undefined;
    }
  ...
  private confirmTx(_, txp, wallet) {
    return new Promise(resolve => {
      ...
      this.txFormatProvider.formatToUSD(wallet.coin, txp.amount).then(val => {
        ...
        // TODO-Revise
        let unit = txp.coin.toLowerCase() === 'btc' ? 'TTS' : txp.coin.toUpperCase();
  ...
  private async openFinishModal(onlyPublish?: boolean) {
    ...
    if (this.callback_url) {
      this.externalLinkProvider.open(this.callback_url);
    }
