...
export class TxDetailsPage {
  ...
  public viewOnBlockchain(): void {
    let btx = this.btx;
    // TODO-Revise (Changed generation of test net URL)
    let url =
      'https://' +
      (this.getShortNetworkName() == 'test' ? [this.blockexplorerUrl.slice(0, this.blockexplorerUrl.indexOf('.')), '-testnet', this.blockexplorerUrl.slice(this.blockexplorerUrl.indexOf('.'))].join('')
        : this.blockexplorerUrl) +
      '/tx/' +
      btx.txid;
    let optIn = true;
