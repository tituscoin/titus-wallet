...
export class HomePage {
  ...
  private _didEnter() {
    // TODO-Revise
    if (this.isNW) this.checkUpdate(false);
    this.checkHomeTip(false);
    this.checkFeedbackInfo(false);
  ...
  private openEmailDisclaimer() {
    let message = this.translate.instant(
      'By providing your email address, you give explicit consent to Titus to use your email address to send you email notifications about payments.'
    );
  ...
  public checkHomeTip(checkHomeTip): void {
    if (checkHomeTip) {
  ...
  private async checkAnnouncement(checkAnnouncement) {
  ...
  private checkFeedbackInfo(checkFeedbackInfo) {
  ...
  private updateAllWallets(): void {
    ...
    setTimeout(() => {
      this.events.publish('AllWalletsUpdated')
    }, 0)
  }

  private checkUpdate(checkUpdate): void {
    if (checkUpdate) {
