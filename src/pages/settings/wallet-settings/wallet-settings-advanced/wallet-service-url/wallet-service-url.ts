...
export class WalletServiceUrlPage {
  ...
  ionViewWillEnter() {
    ...
    this.comment = this.replaceParametersProvider.replace(
      ...
      this.translate.instant(
        "{{appName}} depends on Titus Coin Wallet Service (BWS) for blockchain information, networking and Copayer synchronization. The default configuration points to http://localhost:3232/bws/api."
      ),
      { appName }
    );
  ...
  public save(): void {
    ...
    switch (this.walletServiceForm.value.bwsurl) {
      case 'prod':
      case 'production':
        bws = 'https://tws.tituscoin.com/bws/api';
        break;
      case 'sta':
      case 'staging':
        bws = 'https://tws.tituscoin.com/bws/api';
