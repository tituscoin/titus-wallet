      ...
      this.bitPayProvider.postAuth(
        ...
        data => {
          ...
          this.fetchBasicInfo(apiContext, (err, basicInfo) => {
            ...
            let title = this.translate.instant('Add Titus Account?');
            let msg;
            ...
            if (pairingReason) {
              ...
              msg = this.replaceParametersProvider.replace(
                this.translate.instant(
                  'To {{reason}} you must first add your Titus account - {{email}}'
                ),
                { reason, email }
              );
            } else {
              let email = pairData.email;
              msg = this.replaceParametersProvider.replace(
                this.translate.instant('Add this Titus account ({{email}})?'),
                { email }
              );
            }
  ...
  private checkOtp(pairData, cb: (otp?) => any) {
    if (pairData.otp) {
      let msg = this.translate.instant(
        'Enter Two Factor for your Titus account'
      );
